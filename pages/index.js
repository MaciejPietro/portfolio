  ...
  return (
    <li className="flex gap-4">
      ...
      <div className="relative mt-1 flex h-10 w-10 flex-none items-center justify-center rounded-full shadow-md shadow-zinc-800/5 ring-1 ring-zinc-900/5 dark:border dark:border-zinc-700/50 dark:bg-zinc-800 dark:ring-0">
        <Image src={role.logo} alt="" className="h-7 w-7 rounded-full" fill
      style={{objectFit:"cover"}} unoptimized />
      </div>
      <dl className="flex flex-auto flex-wrap gap-x-2">
        <dt className="sr-only">Company</dt>
        <dd className="w-full flex-none text-sm font-medium text-zinc-900 dark:text-zinc-100">
            {role.url ? <a className="text-teal-500 text-xs hover:opacity-75" href={role.url} target="_blank">{role.company}</a> : role.company}
        </dd>
  ...
  return (
    <>
      <MainLayout preview={preview}>
        ...
        <Hero title={pageContent.heroBanner.headline} description={pageContent.heroBanner.subHeading} body={pageContent.heroBanner.body} links={generalContent} />
